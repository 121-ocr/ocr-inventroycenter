 ...
 */
// 业务活动功能处理器
public class MatchLocationByInvBatchHandler extends ActionHandlerImpl<JsonObject> {

	public static final String ADDRESS = "querylocations";

	public MatchLocationByInvBatchHandler(AppActivityImpl appActivity) {
		super(appActivity);
	...
	public void handle(CommandMessage<JsonObject> event) {
		...
		getLocationsByRule(event.getContent(), ret -> {
	...
	public void getLocationsByRule(JsonObject params, Handler<AsyncResult<JsonArray>> next) {
		...
		params.put("sort", new JsonObject().put("onhandnum", 1));		
