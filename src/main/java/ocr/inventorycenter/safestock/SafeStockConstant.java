package ocr.inventorycenter.safestock;

/**
 * 安全库存 常量类
 * 
 * @author LCL
 *
 */
public class SafeStockConstant {
	// -----------------冗余信息
	public static String bo_id = "bo_id";
	public static String account = "account";
	public static String locations = "locations";
	public static String warehouses = "warehouses";
	public static String goods = "goods";

	// -----------------维度信息

	public static String sku = "sku";
	public static String warehousecode = "warehousecode";
	public static String safenum = "safenum";

	// -----------------组件信息
	public static String ComponentNameConstant = "safestock";
	public static String ComponentBizObjectTypeConstant = "bs_safestock";

	// -----------------handler address
	public static String CreateAddressConstant = "create";
	public static String ModifyAddressConstant = "modify";
	public static String QueryAddressConstant = "query";
	public static String RemoveAddressConstant = "remove";

	// 创建状态
	public static final String CREATE_STATUS = "created";
	public static final String REMOVE_STATUS = "deleted";

}
